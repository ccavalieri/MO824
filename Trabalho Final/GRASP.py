#!/usr/bin/env python3
"""
GRASP for School Dropout Problem
"""
...
class IncrementalEvaluator:
    """
    Incremental evaluator for delta calculations.
    """
...
class GRASP:
    """
    GRASP solver for School Dropout Problem.
    """
    
    def __init__(self, instance, alpha=0.1, iterations=100, 
                 local_search='first', max_candidates_per_student=10,
                 initialization='varied', seed=42, max_time=600,
                 max_iter_no_improvement=100):
        """
        Initialize GRASP solver.
        
        Args:
            instance: SchoolDropoutInstance object
            alpha: Greediness parameter (0=greedy, 1=random)
            iterations: Number of GRASP iterations
            local_search: 'first' or 'best' improving
            max_candidates_per_student: Max subsets to evaluate per student (space reduction)
    ...
    def solve(self):
        """Run GRASP algorithm."""
        print(f"\n{'='*70}")
        print(f"GRASP OPTIMIZED - School Dropout Problem")
        print(f"{'='*70}")
        print(f"Iterations: {self.iterations}")
        print(f"Alpha: {self.alpha}")
        print(f"Local search: {self.local_search.upper()}")
        ...
        for iteration in range(self.iterations):
            ...
            # Construction phase
            solution = self._construction_phase(init_strategy)
            
            # Local search phase
            if self.local_search == 'first':
                solution = self._local_search_first_improving(solution)
            else:
                solution = self._local_search_best_improving(solution)
            ...
            if solution.is_feasible and solution.objective_value < self.best_objective:
                ...
                print(f"  Iter {iteration+1:3d}: New best = {self.best_objective:12.6f} "
                      f"(Risk reduction: {improvement:5.2f}%)")
            else:
        ...
        print(f"\n{'='*70}")
        print(f"GRASP completed in {elapsed_time:.2f} seconds")
    ...
    def _construction_phase(self, init_strategy='empty'):
        """
        Construction phase
        
        Args:
            init_strategy: 'empty', 'random', or 'risk'
        """
        ...
        while candidate_students and iterations_count < max_iterations:
            ...
            rcl_candidates = self._build_rcl_benefit_cost(
                solution, evaluator, candidate_students
            )
            ...
            # Check if we should continue with this student
            # Remove if they received a non-empty subset
            if new_subset_idx != 0:
    ...
    def _initialize_risk_based(self, solution):
        ...
        # Assign something to top 20% highest-risk students
        ...
        for student_id in high_risk_students[:len(high_risk_students)//2]:
            ...
            eligible = [i for i in range(len(self.instance.subsets[student_id])) if i != 0]
            if eligible:
                # Pick one with good effectiveness/cost ratio
                best_idx = 0
                best_ratio = -float('inf')
                
                for idx in eligible[:5]:  # Check first 5
    ...
    def _build_rcl_benefit_cost(self, solution, evaluator, candidate_students):
        """
        Build RCL
        """
        candidates = []
        
        # Sample students to evaluate (don't evaluate all for large instances)
        ...
        for student_id in sampled_students:
            ...
            # Get eligible subsets (prioritize by effectiveness, limit search)
            eligible_subsets = []
            ...
            for new_subset_idx in eligible_subsets:
                ...
                if benefit > 0:  # Only consider improvements
                    ratio = benefit / cost
        ...
        # Build RCL 
        candidates.sort(key=lambda x: x[0], reverse=True)  # Sort by ratio (descending)
        
        best_ratio = candidates[0][0]
        worst_ratio = candidates[-1][0]
        
        threshold = best_ratio - self.alpha * (best_ratio - worst_ratio)
    ...
    def _local_search_first_improving(self, solution):
        """
        First improving local search with 3 neighborhood structures.
        """
        evaluator = IncrementalEvaluator(self.instance, solution)
        improved = True
        ...
        while improved:
            ...
            # Neighborhood 1: Intervention add/remove (N3)
            if not improved:
                improved = self._neighborhood_intervention_add_remove(solution, evaluator, first_improving=True)
            
            # Neighborhood 2: Subset swap (N1)
            if not improved:
                improved = self._neighborhood_subset_swap(solution, evaluator, first_improving=True)
            
            # Neighborhood 3: Student pair swap (N2)
            if not improved:
                improved = self._neighborhood_student_swap(solution, evaluator, first_improving=True)
        
        solution.evaluate()
        return solution
    
    def _local_search_best_improving(self, solution):
        """
        Best improving local search with 3 neighborhood structures.
        """
        evaluator = IncrementalEvaluator(self.instance, solution)
        improved = True
        ...
        while improved:
            ...
            # Neighborhood 1: Intervention add/remove (N3)
            move = self._neighborhood_intervention_add_remove(solution, evaluator, first_improving=False)
            if move and move[0] < best_delta:
                best_delta = move[0]
                best_move = move
            
            # Neighborhood 2: Subset swap (N1)
            move = self._neighborhood_subset_swap(solution, evaluator, first_improving=False)
            if move and move[0] < best_delta:
                best_delta = move[0]
                best_move = move
            
            # Neighborhood 3: Student pair swap (N2)
            move = self._neighborhood_student_swap(solution, evaluator, first_improving=False)
            if move and move[0] < best_delta:
                best_delta = move[0]
                best_move = move
            ...
            if best_move:
                # Apply best move
                if len(best_move) == 3:  # Subset swap or intervention add/remove
                    _, student_id, new_subset_idx = best_move
                    evaluator.apply_move(student_id, new_subset_idx)
                else:  # Student swap
                    _, student1, idx1, student2, idx2 = best_move
                    evaluator.apply_move(student1, idx1)
                    evaluator.apply_move(student2, idx2)
                improved = True
    ...
    def _neighborhood_subset_swap(self, solution, evaluator, first_improving=True):
        """
        Subset swap neighborhood with smart sampling.
        For each student, try changing their assigned subset.
        """
        # Sample students
        ...
        for student_id in students:
            ...
            # Limit number of subsets to try
    ...
    def _neighborhood_student_swap(self, solution, evaluator, first_improving=True):
        """
        Student swap neighborhood with strategic sampling.
        Swap subset assignments between pairs of students.
        """
...
def save_metrics(solution, elapsed_time, output_file):
    ...
    with open(output_file, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("GRASP OPTIMIZED SOLUTION METRICS\n")
        f.write("=" * 70 + "\n\n")
...
def save_solution_details(solution, output_file):
    """Save detailed solution allocation to file."""
    instance = solution.instance
    cost_details = solution.get_cost_details()
    
    with open(output_file, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("DETAILED SOLUTION - INTERVENTION ALLOCATION\n")
        f.write("=" * 70 + "\n\n")
        
        # Activated interventions
        f.write("ACTIVATED INTERVENTIONS\n")
        f.write("-" * 70 + "\n")
        for i in range(instance.n_interventions):
            count = cost_details['intervention_count'][i]
            if count > 0:
                capacity = instance.capacities[i]
                usage_pct = (count / capacity) * 100
                f.write(f"Intervention {i}: {count:4d} students ({usage_pct:5.1f}% capacity)\n")
        f.write("\n")
        
        # Student allocations
        f.write("STUDENT ALLOCATIONS\n")
        f.write("-" * 70 + "\n")
        f.write("Format: student_id | initial_risk | subset | interventions | effectiveness\n")
        f.write("-" * 70 + "\n")
        
        for student_id in range(instance.n_students):
            subset_idx = solution.assignment[student_id]
            subset = instance.subsets[student_id][subset_idx]
            risk = instance.risks[student_id]
            effectiveness = instance.effectiveness[student_id][subset_idx]
            
            if len(subset) == 0:
                interventions_str = "None"
            else:
                interventions_str = ",".join(str(i) for i in sorted(subset))
            
            f.write(f"{student_id:6d} | {risk:.6f} | {subset_idx:3d} | "
                   f"{interventions_str:20s} | {effectiveness:.6f}\n")
        
        f.write("\n" + "=" * 70 + "\n")
    
    print(f"Solution details saved to: {output_file}")


def main():
    parser = argparse.ArgumentParser(
        description="GRASP Optimized for School Dropout Problem",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Stop after N iterations without improvement (default: 100)"
    )
    ...
    if args.output_metrics is None:
        args.output_metrics = f"metrics_grasp{instance_basename}.txt"
    if args.output_solution is None:
        args.output_solution = f"solution_grasp{instance_basename}.txt"
    
    print("=" * 70)
    print("GRASP OPTIMIZED - SCHOOL DROPOUT PROBLEM")
    print("=" * 70)
    ...
    solution, elapsed_time = grasp.solve()
    ...
    #save_solution_details(solution, args.output_solution)
